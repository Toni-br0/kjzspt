...
public class SysDeptController extends BaseController {
    ...
    @GetMapping("hxtree")
    public ResultTree hxtree(SysHx6Tree param) {
        List<SysHx6Tree> data = sysDeptService.hxList(param);
        return dataTree(data);
    }
    ...
    @Log(title = "新增部门", describe = "新增部门", type = BusinessType.ADD)
    @PreAuthorize("hasPermission('/system/dept/add','sys:dept:add')")
     ...
     */
    @PutMapping("update")
    @Log(title = "修改部门", describe = "修改部门", type = BusinessType.EDIT)
    @PreAuthorize("hasPermission('/system/dept/edit','sys:dept:edit')")
    ...
    @Log(title = "删除部门", describe = "删除部门", type = BusinessType.REMOVE)
